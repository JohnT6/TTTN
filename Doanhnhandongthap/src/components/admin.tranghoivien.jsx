            ...
            <div className="relative z-10 w-full max-w-7xl mx-auto flex flex-col gap-12 lg:gap-16">
                ...
                <div className={`flex flex-col gap-8 lg:gap-16 items-center ${layout === 'image_right' ? 'lg:flex-row-reverse' : 'lg:flex-row'}`}>
                    ...
                    <div className="flex-1 w-full flex flex-col gap-6">
                        ...
                        {/* Khối Quyền lợi hội viên */}
                        {(benefitsTitle?.text || (benefits && benefits.length > 0)) && (
                            <div className="mt-4 p-6 bg-[#f8f9fa] rounded-lg shadow-sm flex flex-col gap-2 border border-gray-100">
                                {benefitsTitle?.text && (
                                    <h4 className="mb-4" style={benefitsTitle.config ? getTextStyle(benefitsTitle.config) : { fontWeight: 'bold', color: '#0368B0', fontSize: '18px' }}>
                                        {benefitsTitle.text}
                                    </h4>
                                )}
                ...
                {stats && stats.length > 0 && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 lg:gap-10 mt-8">
                        {stats.map((stat, idx) => (
                            <div key={idx} className="bg-white rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.05)] p-6 lg:p-8 flex flex-col items-center text-center justify-center transition-transform hover:-translate-y-1">
...
export default AdminTrangHoiVien;
